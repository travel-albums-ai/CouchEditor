...
`rgbBlackPointStage(blackR, blackG, blackB)` is the **RGB black point** node. Each channel has a UI range `0..255`, step `1`, default `0`.
...
`rgbWhitePointStage(whiteR, whiteG, whiteB)` is the **RGB white point** node. Each channel has a UI range `0..255`, step `1`, default `255`.
